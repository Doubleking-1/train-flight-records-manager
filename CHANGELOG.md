...
### 新增功能
- **OpenStreetMap (OSM) 支持**：新增第三种地图选项 CartoDB (OpenStreetMap)，提供更多地图选择。
- **跨页面地图同步**：首页和管理页面的地图类型选择通过 localStorage 自动同步，在任一页面切换地图，另一页面刷新后自动同步。
- **动画回放地图同步**：动画回放模态框的地图类型与主页面地图类型自动保持一致。
- **动画回放自动重置**：关闭动画回放时自动重置回放状态（清空路径、重置进度），确保下次打开时状态干净。
- **动画回放地图重载**：打开动画回放时自动销毁旧地图实例并重新初始化以匹配当前地图类型。

### 功能改进
- **主题一致性优化**：
  - 首页高德地图现在在创建时直接应用正确主题，避免短暂的默认样式闪现。
  - 修复了从 OSM 切换到高德地图时主题不一致的问题。
  - 修复了从 OSM（暗色模式）切换到高德地图时，高德地图背景异常变灰的问题（清理 CSS Filter）。
- **地图切换逻辑优化**：
  - 首页地图销毁逻辑改用"鸭子类型"检测，根据实例方法判断地图类型，避免状态不一致。
  - 管理页面 `initialLoad` 函数现在从 localStorage 读取地图类型并初始化对应地图（包括 Leaflet）。
- **Google Maps API 检查**：在初始化谷歌地图前检查 API 可用性，如果不可用则自动回退到高德地图并提示用户。

### 问题修复
- **首页地图切换失败**：修复了从一个地图类型切换到另一个时，新地图无法加载的问题。
- **OSM 路径丢失**：修复了从首页（OSM）进入管理页面后路径不显示的问题（为 Leaflet 添加了 `handleMapLoad` 触发逻辑）。
- **动画回放路径绘制**：修复了动画回放在 Google Maps 和 Leaflet 上不绘制路径的问题（为 `animatePolyline` 函数添加了 Leaflet 分支）。
