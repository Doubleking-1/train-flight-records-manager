# 🚆✈️ Train & Flight Records Manager (v11.0.0)
...
1. 根据您的服务提供商（如 ChatAnywhere）获取 API Key
...
### 🖱️ 交互体验与 UI 增强
- ✨ **全局统一 ESC 关闭** - 全站所有弹窗、遮罩层、菜单均支持 ESC 键一键关闭，交互逻辑更丝滑
- ✨ **线条交互升级** - 地图 Polyline 线路新增右键菜单，支持“查看详情”与“重新画线”
- ✨ **行程详情 Modal** - 精心设计的详情弹窗，自动计算均速，格式化显示“X小时Y分钟”
- ✨ **Bug 修复** - 彻底解决重新画线时的“双重影”重叠问题，确保地图显示清爽
- ✨ **UI 视野优化** - 增加侧边栏宽度，调整首页地图默认缩放（Zoom: 3），在大屏和多数据下表现更佳

### 🏗️ JavaScript 架构重构 (v10.0.0)
- ✨ **模块化架构** - 从单文件重构为专业的四层架构（pages/modules/services/utils）
- ✨ **工具函数提取** - 创建独立的 helpers.js 模块（坐标转换、时长格式化）
- ✨ **统计模块拆分** - 统计功能独立为 statistics.js 模块
- ✨ **代码组织优化** - app.js 减少 265 行，提升可维护性
